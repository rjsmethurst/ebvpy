...
def calc_ebv(ra, dec):
		""" Given sky coordinates (ra, dec) this function calculates the corresponding E(B-V) value on that part of the sky using the
		Schlegel et al. (1998) dust maps. 

	INPUTS
	=-=-=-
	:ra:
		right ascension of sources, in units of degrees. Numpy array of dimensions (N,)
	:dec:
		declination of sources, in units of degrees. Numpy array of dimensions (N,)
	...
	if os.path.exists('~/SFD_dust_4096_ngp.fits') == False:
		...
		pass
	ngp = fits.open('~/SFD_dust_4096_ngp.fits')
	sgp = fits.open('~/SFD_dust_4096_sgp.fits')
...
def calc_color_correction(band, ebv):
		""" Given the E(B-V) value and a photometric bandpass this function calculates the corresponding magnitude correction using pre-defined
		or user defined A_b/E(B-V) values. 
